...
// Set Chunk Size to 50MB to reduce part count and speed up final stitching
...
Deno.serve(async (req) => {
  ...
  if (req.method === "GET" && url.pathname === "/") {
    const html = `
      ...
      <html lang="en">
      <head>
        <title>Bunny Fast Uploader</title>
        ...
        <style>
          ...
          input { width: 100%; padding: 14px; margin-bottom: 15px; box-sizing: border-box; border: 1px solid #dfe6e9; border-radius: 8px; outline: none; font-size: 14px; }
          ...
          textarea { width: 100%; padding: 10px; background: #f1f2f6; border: 1px dashed #b2bec3; border-radius: 6px; margin-top: 10px; font-size: 13px; resize: none; box-sizing: border-box; }
      ...
      <body>
        <div class="card">
          <div id="formSection">
            <h2>🚀 Fast Uploader</h2>
            <p>High speed, 50MB Chunks.</p>
            <form id="uploadForm">
              <input type="url" id="fileUrl" required placeholder="Paste Remote URL..." />
          ...
          <div class="status-container" id="statusSection">
            ...
            <div style="font-size: 12px; color: #b2bec3; margin-top: 5px;">Do not close this tab.</div>
          </div>
              ...
              try {
                 ...
                 if (data.success) {
                   resultSection.innerHTML = \`
                     <h3 style="color:#00b894; margin:0;">✅ Complete</h3>
                     <p style="font-size:13px; margin: 5px 0;">\${data.fileName}</p>
                     <textarea rows="3" onclick="this.select()">\${data.link}</textarea>
                     <button class="btn-restart" onclick="location.reload()">Upload Another</button>
                   \`;
                 } else {
                 ...
                 throw new Error("Upload finished but response was unclear. Check Bunny dashboard.");
              }
  ...
  if (req.method === "POST" && url.pathname === "/upload") {
    ...
    (async () => {
        ...
        try {
            ...
            await sendUpdate("Fetching remote file...");
            ...
            await sendUpdate("Starting Upload...");
            ...
            while (true) {
                ...
                if (buffer.length >= CHUNK_SIZE || (done && buffer.length > 0)) {
                    while (buffer.length >= CHUNK_SIZE || (done && buffer.length > 0)) {
                        ...
                        // Retry Logic (3 attempts per part)
                        let uploaded = false;
                        let attempts = 0;
                        while (!uploaded && attempts < 3) {
                            try {
                                ...
                                console.error(e);
                                await sendUpdate(`Retry Part ${partNumber} (${attempts}/3)...`);
            ...
            await sendUpdate("Finalizing (Stitching)...");
